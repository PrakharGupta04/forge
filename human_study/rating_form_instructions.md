...
You will evaluate **30 AI agent execution traces** and rate each one on **5 dimensions**. Each trace shows the task, the agent's reasoning and tool-use steps, and its final answer. Rate the agent's *process and output* — not how you would have solved the task yourself.
